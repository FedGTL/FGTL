...
class Server(object):
    def __init__(self, args, encoder: nn.Module, classifier: nn.Module, c_global_encoder: nn.Module, c_global_classifier: nn.Module):
        self.args = args
        self.encoder = encoder
        self.classifier = classifier
        self.c_global_encoder = c_global_encoder
        self.c_global_classifier = c_global_classifier
    ...
    def send_global_c_encoder(self):
        return copy.deepcopy(serialize(self.c_global_encoder.state_dict()))

    def send_global_c_classifier(self):
        return copy.deepcopy(serialize(self.c_global_classifier.state_dict()))
    ...
    def aggregation(self):
        ...
        total_encoder_delta = {}
        total_classifier_delta = {}
        n_total_selected = len(self.current_round_parames)
        for paraminfo in self.current_round_parames:
            if 'c_encoder_delta_params' in paraminfo.keys():
                for k,v in deserialize(paraminfo['c_encoder_delta_params']).items():
                    if k not in total_encoder_delta:
                        total_encoder_delta[k] = []
                    
                    total_encoder_delta[k].append(v.cpu().numpy())

            if 'c_classifier_delta_params' in paraminfo.keys():
                for k,v in deserialize(paraminfo['c_classifier_delta_params']).items():
                    if k not in total_classifier_delta:
                        total_classifier_delta[k] = []
                    
                    total_classifier_delta[k].append(v.cpu().numpy())
        
        if len(total_encoder_delta) != 0:
            c_global_encoder_para = self.c_global_encoder.state_dict()
            for key in c_global_encoder_para:
                c_global_encoder_para[key] += torch.tensor(np.array(total_encoder_delta[key])).sum(dim=0) / n_total_selected
            self.c_global_encoder.load_state_dict(c_global_encoder_para)

        if len(total_classifier_delta) != 0:
            c_global_classifier_para = self.c_global_classifier.state_dict()
            for key in c_global_classifier_para:
                c_global_classifier_para[key] = c_global_classifier_para[key].float()
                c_global_classifier_para[key] += torch.tensor(np.array(total_classifier_delta[key])).sum(dim=0) / n_total_selected
            self.c_global_classifier.load_state_dict(c_global_classifier_para)
...
class Client(object):
    def __init__(self, args, client_id, graph, num_features,
        ...
        self.c_local_encoder = copy.deepcopy(encoder)
        self.c_global_encoder = copy.deepcopy(encoder)
        self.c_local_classifier = copy.deepcopy(classifier)
        self.c_global_classifier = copy.deepcopy(classifier)
    ...
    def client_update(self):
        ...
        self.c_global_encoder.to(self.args.gpu)
        self.c_local_encoder.to(self.args.gpu)
        self.c_global_classifier.to(self.args.gpu)
        self.c_local_classifier.to(self.args.gpu)
        ...
        c_global_encoder_para = self.c_global_encoder.state_dict()
        c_global_classifier_para = self.c_global_classifier.state_dict()
        c_local_encoder_para = self.c_local_encoder.state_dict()
        c_local_classifier_para = self.c_local_classifier.state_dict()
        ...
        for i in range(self.args.local_epoch):
            ...
            # for scaffold
            net_encoder_para = self.encoder.state_dict()
            net_classifier_para = self.classifier.state_dict()

            for key in net_encoder_para:
                net_encoder_para[key] = net_encoder_para[key] - self.args.lr * (c_global_encoder_para[key] - c_local_encoder_para[key])
            self.encoder.load_state_dict(net_encoder_para)

            for key in net_classifier_para:
                net_classifier_para[key] = net_classifier_para[key] - self.args.lr * (c_global_classifier_para[key] - c_local_classifier_para[key])
            self.classifier.load_state_dict(net_classifier_para)

            losses.append(loss.item())

        c_new_encoder_para = copy.deepcopy(self.c_local_encoder.state_dict())
        c_new_classifier_para = copy.deepcopy(self.c_local_classifier.state_dict())
        c_encoder_delta_para = copy.deepcopy(self.c_local_encoder.state_dict())
        c_classifier_delta_para = copy.deepcopy(self.c_local_classifier.state_dict())
        global_encoder_para = global_encoder.state_dict()
        global_classifier_para = global_classifier.state_dict()
        net_encoder_para = self.encoder.state_dict()
        net_classifier_para = self.classifier.state_dict()
        
        for key in net_encoder_para:
            c_new_encoder_para[key] = c_new_encoder_para[key] - c_global_encoder_para[key] + (global_encoder_para[key] - net_encoder_para[key]) / (self.args.local_epoch * self.args.lr)
            c_encoder_delta_para[key] = c_new_encoder_para[key] - c_local_encoder_para[key]
        self.c_local_encoder.load_state_dict(c_new_encoder_para)
        
        for key in net_classifier_para:
            c_new_classifier_para[key] = c_new_classifier_para[key] - c_global_classifier_para[key] + (global_classifier_para[key] - net_classifier_para[key]) / (self.args.local_epoch * self.args.lr)
            c_classifier_delta_para[key] = c_new_classifier_para[key] - c_local_classifier_para[key]
        self.c_local_classifier.load_state_dict(c_new_classifier_para)
        ...
        paraminfo = dict(
            ...
            c_encoder_delta_params = serialize(c_encoder_delta_para),
            c_classifier_delta_params = serialize(c_classifier_delta_para)
        )
...
class Scaffold(object):
    def __init__(self, args):
        ...
        self.server = Server(self.args, 
            ...
            c_global_encoder=c_global_encoder,
            c_global_classifier=c_global_classifier
        )
    ...
    def send_global_c(self, is_init: Optional[bool] = False):
        if is_init:
            for k, client in enumerate(self.clients):
                client.c_local_encoder.load_state_dict(deserialize(self.server.send_global_c_encoder()))
                client.c_local_classifier.load_state_dict(deserialize(self.server.send_global_c_classifier()))
        else:
            for k, client in enumerate(self.clients):
                client.c_global_encoder.load_state_dict(deserialize(self.server.send_global_c_encoder()))
                client.c_global_classifier.load_state_dict(deserialize(self.server.send_global_c_classifier()))
    ...
    def train(self):
        ...
        self.send_global_c(is_init=True)
