...
from utils import *
from model import FGNN_model, NetworkEmbedding, models
...
class Server(object):
    def __init__(self, args, model: nn.Module):
        ...
        self.filter_keywords = ['domain_discriminators', 'domain_student']
...
class Client(object):
    ...
    def client_update(self, is_target: bool = False):
        ...
        self.model.train()
        # Freeze student domain discriminator parameters
        for param in self.model.domain_student.parameters():
        ...
        for i in range(self.args.local_epoch):
            ...
            for batch in self.data_loader:
                ...
                emb, pred_logit, d_logit, stu_logit = self.model(batch.x, batch.edge_index, batch.edge_weight)
                ...
                total_loss += self.args.domain_coef * F.cross_entropy(stu_logit[:batch_size], domain_label)

                total_loss += self.args.domain_coef * F.cross_entropy(d_logit[batch.s_t_mask[:batch_size]], domain_label[batch.s_t_mask[:batch_size]])
        ...
        # Unfreeze student domain discriminator parameters
        for param in self.model.domain_student.parameters():
            param.requires_grad = True

        opt_stu = optim.Adam(self.model.domain_student.parameters(), lr=self.args.lr, weight_decay=self.args.weight_decay)
        
        self.graph.to(self.args.gpu)
        embeddings = self.model.network_embedding(self.graph.x, self.graph.edge_index, self.graph.edge_weight)
        test_data = embeddings.detach_()[self.graph.s_t_mask == False]
        if is_target:
            domain_label = torch.ones(test_data.shape[0]).long().to(self.args.gpu)
        else:
            domain_label = torch.zeros(test_data.shape[0]).long().to(self.args.gpu)
        
        teacher_output = self.model.domain_discriminators.predict(test_data)
        teacher_predict = torch.tensor(teacher_output['predictions']).to(self.args.gpu)
        # self.logger.info('teacher acc: {}'.format(accuracy(teacher_predict, domain_label)))

        for i in range(self.args.local_epoch):
            logits = self.model.domain_student(test_data)
            loss = F.cross_entropy(logits, teacher_predict)
        ...
        if is_target:
            state = param_filter(self.model.state_dict(), ['node_classifier', 'domain_discriminators', 'domain_student'])
        else:
            state = param_filter(self.model.state_dict(), ['domain_discriminators', 'domain_student'])
        ...
        return np.mean(losses), paraminfo, serialize(self.model.domain_student.state_dict())

    
    def dm_update(self, dm_param):
        self.model.domain_student.load_state_dict(deserialize(dm_param), strict=False)
    ...
    def client_eval(self, istest=False):
        ...
        with torch.no_grad():
            ...
            _, logits, _, _ = self.model(self.graph.x, self.graph.edge_index, self.graph.edge_weight)
...
class FGNN(object):
    def __init__(self, args):
        ...
        for i in range(self.num_clients):
            ...
            # for pate
            s_t_idx = np.arange(self.subgraphs[i].num_nodes)
            random.shuffle(s_t_idx)
            s_t_mask = torch.zeros(self.subgraphs[i].num_nodes, dtype=torch.bool)
            s_t_mask[s_t_idx[:int(self.subgraphs[i].num_nodes * 0.7)]] =True
            self.subgraphs[i].s_t_mask = s_t_mask
