...
class Client(object):
    ...
    def client_update(self):
        ...
        if self.prev_encoder != None:
            self.prev_encoder.to(self.args.gpu)

        global_encoder = copy.deepcopy(self.encoder)
        global_encoder.eval()
        for param in global_encoder.parameters():
        ...
        for i in range(self.args.local_epoch):
            ...
            if self.prev_encoder != None:
                global_logits = global_encoder(self.graph.x, self.graph.edge_index)
                prev_logits = self.prev_encoder(self.graph.x, self.graph.edge_index)

                posi =cos(embeddings, global_logits)
                nega = cos(embeddings, prev_logits)

                con_logits = torch.cat((posi.reshape(-1, 1), nega.reshape(-1, 1)), dim=1)
                con_logits /= self.args.temperature

                con_labels = torch.zeros(self.graph.x.size(0)).to(self.args.gpu).long()

                con_loss = self.args.mu * F.cross_entropy(con_logits, con_labels)    
                loss += con_loss
        ...
        for param in self.prev_encoder.parameters():
            param.requires_grad = False
